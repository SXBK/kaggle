...
rfModel = GradientBoostingRegressor(n_estimators=4000, alpha=0.01)
rfModel.fit(x_train, np.log1p(y_train))
preds=rfModel.predict(X= x_train)
preds=[int(np.exp(x)) for x in preds]
...
#preds=[x*countStd+countMean for x in preds]
print(preds[:5])
submission=pd.DataFrame({"datetime":test['datetime'],"count":[max(0, x) for x in np.exp(preds)]})
